...
void initializeSystem() {
    ...
    TRISBbits.RB0 = IN;
    TRISBbits.RB1 = IN;
    TRISBbits.RB3 = IN; // Nuevo botón en RB3 para START/RESET
    TRISCbits.RC0 = OUT;
...
void checkCollision() {
    // Colisión con cactus (dinosaurio en el suelo, fila 1)
    if (dinosaurioPos == 1 && cactusPos == 0) {
        ...
        // Redibujar la línea vertical en la columna 12
        LCDMoveCursor(12, 0);
        LCDchar('|');
        LCDMoveCursor(12, 1);
        LCDchar('|');
    ...
    // Colisión con pájaro (dinosaurio en el aire, fila 0)
...
void check_buttons() {
    ...
    if (Boton3 == ON) {  
        __delay_ms(50);
        if (Boton3 == ON) {
            while (Boton3 == ON); // Esperar a que se suelte el botón
            resetGame();
        }
    }
}
